...
function display_random_loading_text(element_id) {
    const randomNumber = Math.random();
    let chosenItem;

    if (randomNumber < 0.5) {
        const randomIndex = Math.floor(Math.random() * loading_text_rank_1.length);
        chosenItem = loading_text_rank_1[randomIndex];
    } else if (randomNumber < 0.75) {
        const randomIndex = Math.floor(Math.random() * loading_text_rank_2.length);
        chosenItem = loading_text_rank_2[randomIndex];
    } else {
        const randomIndex = Math.floor(Math.random() * loading_text_all.length);
        chosenItem = loading_text_all[randomIndex];
    }

    // Display the chosen item in a specific part of your webpage
    document.getElementById(element_id).textContent = chosenItem;
}
